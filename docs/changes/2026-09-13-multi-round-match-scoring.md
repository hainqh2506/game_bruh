...
- `game/room.py`: Cập nhật `Player` (điểm, thời gian), `RoomSession` (state machine nhiều vòng, scoring, ranking tie-break, next_round, sweep_stale time limit).
...
- `play/room.js` & `play/room.css`: Thêm dropdown chọn số câu, thời gian, đồng hồ đếm ngược, badge điểm số, banner kết thúc vòng và bảng vinh danh chung cuộc.
- `tests/test_room.py`: Bổ sung unit tests cho tính điểm, chuyển vòng, tie-breaker và trần thời gian.
